    ...
    formSubmit(event:any) {
        ...
        if(this.props.parameters){            
            for(let i = 0; i < this.props.parameters.length; i++){
                url = url.replace('{'+this.props.parameters[i].key+'}', event.target.elements[this.props.parameters[i].key].value);
            }
        ...
        if(this.props.body){
            ...
            for(let i = 0; i < this.props.body.length; i++){
                axios_config.data[this.props.body[i].name] = event.target.elements[this.props.body[i].name].value;
            }
        }

        axios(axios_config).then((resp: any) => {
        ...
        return (
            <div>
                <form onSubmit={this.formSubmit.bind(this)}>
                    ...
                    <h3 className="explorer-method m-b-s">{this.props.method}</h3>
                    <h3 className="explorer-base m-b-s">Base URL</h3>                
                    <p className="explorer-url"><strong>{this.props.url}</strong></p>                    
                    { this.props.parameters ?
                        <div className="explorer-parameters m-t-m"> 
                        <h3 className="explorer-base m-b-s">URL Parameters</h3>
                        {
                                    ...
                                    <div key={index} className="field">
                                        <div className="field">
                                            <div className="control">
                                                <label className="input-label" htmlFor={obj.name}>{obj.name.replace(/^\w/, (c:string) => c.toUpperCase())}</label>
                                                <input
                                                    className="input"
                                                    type='text'
                    ...
                    <button className="button is-info m-t-m" type="submit">Send Request</button>
